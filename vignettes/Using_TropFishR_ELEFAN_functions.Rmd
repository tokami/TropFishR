...
As an illustration of the fitted soVBGF parameters, the instantaneous values of *K* through time can be calculated according to @munro_estimation_1982. The values oscillate around the mean fitted value, `K`, with `ts` reflecting the time of year when instantaneous K is at a maximum.   
...
In the case of the `alba` dataset, no improvement in Rn is observed for the soVBGF fit (`alba5$Rn_max`) over that of the VBGF fit (`alba4$Rn_max`). Unformtunately, no statistical test is (yet) available for selecting one model over the other. Other information about the species and its environment may be required in order to justify the inclusion of seasonally-oscillating growth; for example, does the period of stronger growth (`ts`) coincide with known changes in the environment? In cases where fitting the soVBGF does not markedly improve Rn, selecting the simpler VBGF model may be advisable.
...
In this case, the *true* values are very well estimated, and the solution even has a higher Rn than that calculated with the true soVBGF parameters:
