...
bibliography: Using_TropFishR_ELEFAN_functions.bib
---
...
# Introduction
The following tutorial outlines the use of the ELEFAN functions available in TropFishR [@mildenberger_tropfishr_2017; @mildenberger_tropfishr:_2017] (`ELEFAN`, `ELEFAN_GA`, and `ELEFAN_SA`). These functions can be used to estimate growth model parameters from length-frequency (lfq) data. The ability to convert length to age is the initial step in length-based stock assessment, and underpins subsequent analyses (see the [TropFishR tutorial](https://cran.r-project.org/package=TropFishR/vignettes/tutorial.html) for a broader overview of length-based stock assessment).
...
Depending on the restructuring settings, the distribution and amplitude of positive and negative scoring bins will be effected. The scoring procedure used in TropFishR follows that of FiSAT II, as outlined by @gayanilo_fao-iclarm_1997. For a given set of VBGF parameters, yearly repeating growth curves are drawn through the lfq data. Scoring is based on the concept of "peaks", which are runs of positive or negative scoring bins. Each peak takes the value of the largest absolute value contained within. 
...
Another approach may be to inform first guesstimates of Linf from other data sources, and allow ELEFAN a significant range of values for searching during VBGF fitting. @froese_empirical_2000 also showed strong correlation between the largest individuals (Lmax) and Linf, $ \log(L_\infty) = 0.044 + 0.9841 * \log(Lmax) $, but Lmax may also be difficult to obtain in highly exploited populations.   
...
The dataset is significantly more detailed (72 length bins, 25 time samples) than `alba` (14 length bins, 7 time samples), and thus will take longer to solve. For this reason a parallelized search was used to speed computation time.  From the previous work presented by @taylor_extending_2017, we know that a moving average setting of `MA = 11` is appropriate. 
