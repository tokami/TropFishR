...
$$ \log(L_\infty) = 0.044 + 0.9841 * \log(Lmax) $$
...
The dataset is significantly more detailed (72 length bins, 25 time samples) than `alba` (14 length bins, 7 time samples), and thus will take longer to solve. For this reason a parallelized search was used to speed computation time.  From the previous work presented by @taylor_extending_2017, we know that a moving average setting of `MA = 11` is appropriate for this lfq data. 
