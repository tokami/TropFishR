...
#'    allows to perform different methods: Gulland and Holt, Ford Walford plot, or
#'    Chapman's method.
#'
...
#'    "FordWalford", "Chapman", or "BertalanffyPlot".
#'
#' @examples
...
#' growth_length_age(dat, method = "BertalanffyPlot", Linf = 50)
#'
#' @details
#'
...
#' used for a small or large sample, respectively.
#'
#' @references
...
growth_length_age <- function(param, method, Linf = NA){
  ...
  switch(method,
         ...
         "BertalanffyPlot"={
           ...
           if(is.na(Linf)) stop("For the Bertalanffy plot you have to provide an estimate for Linf!")
           y <- -log(1 - Lt / Linf)
           x <- t
  ...
  ret <- c(res,list(delta_t = delta_t,
                    x = x,
                    y = y,
                    reg_coeffs = sum_mod$coefficients[1:2],
                    K = K,
                    Linf = Linf))
  return(ret)
}
