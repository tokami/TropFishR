#' @name trout
#' @title Trout data
#'
#' @description This dataset contains information about coral trout
#'  (\emph{Plectropomus leopardus}) caught near Heron Island (Great Barrier Reef,
#'  Australia) in October 1971.
#'  It can be used for the estimation of growth parameters,
#'   \code{\link{ELEFAN}}.
#'
#' @docType data
#' @format A list consisting of: 1. a vaector with midlengths of size classes,
#'  2. a vector with catch in numbers, ...
#'
#' @source Goeden, G.B., 1978. A monograph of the coral trout \emph{Plectropomus leopardus}
#' (Lacépède). \emph{Res.Bull.Fish.Serv.Queensl.}, (1):42 p.
#'
#' Sparre, P., Venema, S.C., 1998. Introduction to tropical fish stock assessment.
#' Part 1. Manual. FAO Fisheries Technical Paper, (306.1, Rev. 2). 407 p.
#'
#' @usage data(trout)
#' @keywords data dataset trout
#' @examples
#'
#' data(trout)
#' str(trout)
#'
#'
NULL
