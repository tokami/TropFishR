#' @name synCAA1
#'
#' @title Synthetic Catch-at-age data I from Sparre & Venema (1998)
#'
#' @description Synthetic Catch-at-age data I. Can be used for the calculation of Z by means of
#'    \code{\link{Z_BevertonHolt}}.
#'
#' @docType data
#'
#' @format A list consisting of:
#' \itemize{
#'   \item \strong{midAge}: a vector of the mid ages of the age groups,
#'   \item \strong{catch_mat}: a matrix with the catches for different years.
#' }
#'
#' @source Sparre, P., Venema, S.C., 1998. Introduction to tropical fish stock assessment.
#'    Part 1. Manual. FAO Fisheries Technical Paper, (306.1, Rev. 2). 407 p.
#' @usage data(synCAA1)
#' @keywords data age Catch-at-age CAA
#'
#' @examples
#' data(synCAA1)
#' str(synCAA1)
#'
#'
NULL
